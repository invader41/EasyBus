...
@interface FavoriteBusCell()<UIGestureRecognizerDelegate>
{
    int _busIndex;
}
...
@implementation FavoriteBusCell
    ...
    switch (recognizer.state) {
        ...
        case UIGestureRecognizerStateChanged: {
               ...
               || self.topContentLeftConstraint.constant != 0)
            {
                ...
                if(deltaX<=kBounceValue && deltaX > 0)
                {
...
//    }
    ...
    NSFetchRequest *search = [NSFetchRequest fetchRequestWithEntityName:@"FavoriteBusLine"];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"busCode==%@ and alarmStationName !=%@", bus.code, @""];
    [search setPredicate:predicate];
    NSArray *result = [SharedAppDelegate.managedObjectContext executeFetchRequest:search error:nil];
    if(result.count > 0)
    {
        [self.alarmImageView setHidden:NO];
    }
    else
    {
        [self.alarmImageView setHidden:YES];
    }
